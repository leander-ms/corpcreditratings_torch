...
def import_data():
    df_creditratings = pd.read_csv(os.path.join('input', 'corporateCreditRatingWithFinancialRatios.csv'))
    ...
    drop = ['Company', 'Company_source']
    df = df.drop(drop, axis=1)
    df = df.rename(columns={'Rating_y': 'ESG Rating', 'Rating_x': 'Rating'}).dropna()

    # get average sector ESG value for NULL ESG scores
    # df_map = df[['Sector', 'ESG Rating']].groupby('Sector').mean(numeric_only=True)

    # nulls = df.loc[df['ESG Rating'].isna(), 'Sector']
    # df.loc[df['ESG Rating'].isna(), 'ESG Rating'] = df_map.loc[nulls].values

    avg_esg_by_sector = df[['Sector', 'ESG Rating']].groupby('Sector').mean(numeric_only=True)

    avg_esg_by_sector.rename(columns = {'ESG Rating': 'Average Sector ESG'}, inplace=True)
    df = pd.merge(df, avg_esg_by_sector, how='left', on='Sector')
    df['ESG to Sector Average'] = df['ESG Rating'] / df['Average Sector ESG']   
...
if __name__ == '__main__':
    ...
    for epoch in range(n_epochs):
        ...
        if acc > best_acc:
            ...
            torch.save(model.state_dict(), os.path.join('torch_weights', 'best_weights_esg.pt'))
    ...
    plt.savefig(os.path.join('torch_eval', 'esg_model', 'test_train_loss.png'))
    ...
    plt.legend()
    plt.savefig(os.path.join('torch_eval', 'esg_model', 'test_train_acc.png'))
    ...
    df_matrix.to_csv(os.path.join('torch_eval', 'esg_model', 'cf_matrix.csv'), sep=';', decimal=',')


    disp = ConfusionMatrixDisplay.from_predictions(y_test_uncoded, y_pred_uncoded, labels=sorted).plot(xticks_rotation=90)
    
    plt.savefig(os.path.join('torch_eval', 'esg_model', 'cf_matrix_display.png'), bbox_inches='tight')
    ...
    cols_to_keep.append('Sector')
    features_german = translate_features(cols_to_keep)

    df_report.to_excel(os.path.join('torch_eval', 'esg_model', 'classification_report.xlsx'))
    ...
    plt.savefig(os.path.join('torch_eval', 'esg_model', 'permutation_importance.png'), bbox_inches='tight')
